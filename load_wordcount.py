...
def read_data():
    ...
    train_x, train_y = data_train_wc['tweet'].copy(), data_train_wc[
        'region'].copy()  # data_train_glove300.iloc[:, [1,2]].copy(),data_train_glove300['region'].copy()
    test_x = data_test_wc['tweet'].copy()
    validation_x, validation_y = data_vali_wc['tweet'].copy(), data_vali_wc['region'].copy()
    max_len = max(
