...
def saveResult(data, name):
    ...
    df['region'] = data
    df['id'] = df.index + 1
    df.to_csv(resultPath + '/glove_funcresult' + name + '.csv', index=None)
...
if __name__ == '__main__':
    train_x, train_y, validation_x, validation_y, test_x = load_glove300.read_data()
    ...
    validation_x = pd.DataFrame(selector.transform(validation_x))
    test_x = pd.DataFrame(selector.transform(test_x))
    print(train_x)
    print(train_x.shape)
    produce_result("naive_bayes", GaussianNB(), train_x, train_y, validation_x, validation_y, test_x)
