...
def saveResult(data, name):
    ...
    df['region'] = data
    df['id'] = df.index + 1
    df.to_csv(resultPath + '/tfidf_DNNresult' + name + '.csv', index=None)
...
if __name__ == '__main__':
    ...
    vocab = readVocab()
    train_x, train_y, validation_x, validation_y, test_x, le = getdata(data_train_tfidf, data_vali_tfidf, data_test_tfidf)
